...
Now do the same against the deployed Function. The MCP Extension uses an
SSE + JSON-RPC transport, so the easiest way to drive a single tool call by hand
is the [MCP Inspector](https://github.com/modelcontextprotocol/inspector):

```bash
npx @modelcontextprotocol/inspector
```

Point it at `https://<your-func>.azurewebsites.net/runtime/webhooks/mcp?code=<mcp_extension-system-key>`,
open the **Tools** tab, and invoke `get_recent_deployments` with
`{ "repo": "shop-api", "branch": "main" }`.
...
export MCP_SERVER_URL="https://<your-func>.azurewebsites.net/runtime/webhooks/mcp?code=<mcp_extension-system-key>"
dotnet run -- "The latest deployment of shop-api to main is failing. Investigate and roll back if needed."
```

> The agent prompt is passed as a command-line argument so it runs a single turn
> and exits. Running `dotnet run` with no argument drops you into an interactive
> prompt instead.

Watch the Function logs in Application Insights (Flex Consumption plans have no
Kudu log stream). You should see three tool calls fire in sequence —
`get_recent_deployments`, `diagnose_deployment`, `create_rollback_pr`.
...
These are the failure modes most often seen when teams set up MI auth to Azure DevOps.
...
### Service principals default to Stakeholder access

Service principals added to an AzDO org default to the **Stakeholder** access
level, which cannot read Code/Repos (you'll get 404s on `/git/repositories/...`
endpoints even when the MI is in Contributors). Upgrade the identity to **Basic**
(express) access in **Organization settings → Users**.

### The MI can authenticate but gets an empty builds list

Some AzDO REST endpoints expect the repo's GUID, not its name. Resolve the
name → GUID first:
